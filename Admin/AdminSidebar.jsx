...
export default function AdminSidebar() {
  ...
  return (
        <>
            ...
            <section
                className={`h-screen bg-white relative transition-all origin-left duration-300 ${isOpen ? 'scale-100' : 'scale-0 w-0'}`}>
                    ...
                    <div className="overflow-y-auto tree-container">
                        <div className="p-10 pb-9">
                            <a href="/home" className="block">
                                <Image src="/whiteLogo.webp" width={300} height={80} alt="logo"/>
                            </a>
                        </div>
                            ...
                            <nav className="px-[25px]">
                                <ul>
                                    ...
                                    <NavItem
                                        ...
                                        submenu
                                    >
                                        <DropdownItem link="/admin/payments/shareholder" menu="Hissedar"/>
                                        <DropdownItem link="/admin/payments/career" menu="Kariyer"/>
                                        <DropdownItem link="/admin/payments/other" menu="Diğer"/>
                                    </NavItem>
...
function NavItem({ menu, link, submenu, message, icon, children }) {
  const [dropdownOpen, setDropdownOpen] = useState(false)

  const trigger = useRef(null)
  const dropdown = useRef(null)

  const setDropdownHandler = (value) => {
    if (!submenu)
      return
  ...
  return (
        <li className="relative">
            <a
                href={link}
                ref={trigger}
                onClick={() => setDropdownHandler(!dropdownOpen)}
                className={`${
                    dropdownOpen ? 'bg-white bg-opacity-10 text-opacity-100' : ''
                } relative flex w-full items-center rounded py-[10px] px-[15px] text-base font-medium text-white text-opacity-50 hover:bg-white hover:bg-opacity-10 hover:text-opacity-100 cursor-pointer`}
            >
                <span className="mr-[10px]">{icon}</span>
                {menu}
                {message && (
                    <span
                        className="ml-3 mt-[2px] flex h-[18px] w-[18px] items-center justify-center rounded-full bg-[#FF3D3D] text-[10px] font-semibold leading-none text-white">
            {message}
          </span>
                )}
                {submenu && (
            ...
            <div
                ref={dropdown}
                onFocus={() => setDropdownHandler(true)}
                onBlur={() => setDropdownHandler(false)}
...
function DropdownItem({ link, menu }) {
  return (
        <li>
            <a
                href={link}
